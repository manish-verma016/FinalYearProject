...
// Manual dotenv parser for local testing environments
try {
    ...
    return {
      emails: {
        send: async (payload) => {
          ...
          return { data: { id: "mock_" + Math.random().toString(36).substring(2, 11) }, error: null };
        }
      }
    };
  }
...
app.post("/api/send-invitation", async (req, res) => {
  const { guests, cardData, inviteLink } = req.body;
